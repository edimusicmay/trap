...
const progressY = 20;
const margin = 10;
const barStartX = BASE_WIDTH * 0.50;
const barEndX = BASE_WIDTH - 60;
...
ctx.fillStyle = white;
ctx.textAlign = "left";
ctx.fillText("Start", barStartX, 40);


ctx.textAlign = "right";
ctx.fillText("End", BASE_WIDTH - margin, progressY + 20);

  // Draw lightsaber hilt
if (hiltLoaded) {
  const hiltHeight = 30; // ← Add this line to fix the error
  const hiltAspect = hiltImage.width / hiltImage.height;
  const hiltWidth = hiltHeight * hiltAspect;
  if (hiltLoaded && hiltCanvas) {
  ctx.drawImage(hiltCanvas, barStartX - cachedHiltWidth - 5, 10, cachedHiltWidth, cachedHiltHeight);
}
...
ctx.fillRect(barStartX, progressY, barWidth * progress, 5);
clearGlow();
...
holdScore[note.lane] = expectedScore;
...
for (let i = 0; i < 4; i++) {
  ...
  if ((holdStillActive || holdFadingOut) && holdScore[i] > 0) {
    ctx.font = '14px Arial';
    ctx.fillText("Long Note +" + holdScore[i], x, hitboxTopBase + hitboxHeight + 20);
  }
}
...
if (ctx && !gameEnded) {
  ...
  if (multiplierBroken && timeSinceReset < 2000 && lastMultiplier > 1) {
  const text = "Perfect streak: x" + lastMultiplier;
  ctx.fillStyle = "#888";
  ctx.font = `bold ${16 + lastMultiplier}px Arial, sans-serif`;
  ctx.fillText(text, 10, multiplierY);

  const textWidth = ctx.measureText(text).width;
  const lineY = multiplierY + 8;
  ctx.strokeStyle = "#888";
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(10, lineY);
  ctx.lineTo(10 + textWidth, lineY);
  ctx.stroke();
...
ctx.fillStyle = `hsl(${hue}, 100%, ${lightness}%)`;
  ...
  ctx.fillText("Perfect streak: x" + multiplier, 10, multiplierY);
...
scoreChangeText[lane] = "+" + baseScore;
...
lastMultiplier = multiplier > 1 ? multiplier : lastMultiplier;
multiplier = 1;
multiplierStreak = 0;
