...
if (window.DeviceMotionEvent) {
  ...
  if (totalAccel > 15 && now - lastShakeTimeMobile > 1000 && isAnyHoldActive()) {
    lastShakeTimeMobile = now;
    shakeBonusActive = true;
    lastShakeTime = performance.now();
    console.log("Shake bonus activated during long note!");
  }
});
...
if (expectedBaseScore > note.holdScore) {
  ...
  holdScore[note.lane] += finalDeltaScore;  // so popup shows total awarded
...
document.addEventListener('keydown', (e) => {
  ...
  if (isAnyHoldActive()) {
    shakeBonusActive = true;
    lastShakeTime = performance.now();
    console.log("Shake bonus activated with Q during long note!");
  } else {
    console.log("Ignored Q: no long note being held.");
  }
}
